...
- **Hobbyist GPU cluster on consumer ethernet.** Two desktop machines wired with standard home networking to run a split model. ✅ **1.69× dual-lane wall-clock on 1 Gbit ethernet** in the controlled microbench ([`poc/09`](poc/09_dual_lane.py)). ✅✅ **Validated end-to-end** via [comfyui-mesh](https://github.com/shootthesound/comfyui-mesh): FLUX.2 Klein 9B at 1024² generates in **4.38 s with NVENC vs 7.20 s raw** (1.64× faster image-to-image) on an RTX 5090 + RTX 4090 over plain gigabit ethernet; at 1536² the gap widens to **4.41 s vs 9.13 s** (2.07×). The codec's wall-clock barely scales with resolution while raw scales linearly with the wire — exactly the regime where compression dominates.
...
- **Long-context KV-spill** — At 64K / 128K context, KV cache exceeds VRAM and spills to system RAM; pulling it back across PCIe per token drops decode to ~3 tok/s on a 32B model. Compressing KV at ~3× lossless gives a direct ~3× boost on the bandwidth-bound side, taking the same workload to ~9 tok/s. ⚠️ compression ratio measured (2.7× lossless on Mistral 7B / 1024-channel KV — see [`docs/findings.md`](docs/findings.md)); end-to-end tok/s benchmark on a real LLM decode loop is the next integration step. *The diffusion equivalent of "end-to-end on consumer ethernet" is now real and reproducible — see the comfyui-mesh numbers above — which is the strongest available evidence that the codec primitive holds up in a real workflow, not just microbenchmarks.*
...
- **Hobbyist GPU cluster on consumer ethernet (1 Gbit / 10 Gbit)** — Two desktop machines (e.g. a 5090 + a 4090) wired together with standard home networking to run a split model. ✅ **1.69× dual-lane wall-clock on 1 Gbit** measured in [`poc/09`](poc/09_dual_lane.py); larger compression ratios + lossy modes give more. ✅✅ **End-to-end validated**: [comfyui-mesh](https://github.com/shootthesound/comfyui-mesh) splits FLUX.2 across an RTX 5090 + RTX 4090 over gigabit ethernet and generates a 1024² image in 4.38 s (NVENC) vs 7.20 s (raw, no codec) — 1.64× faster end-to-end, widening to 2.07× at 1536². (Thunderbolt networking — see the headline section above — gives substantially more headroom than ethernet for the same kind of setup.)
...
| Gigabit cluster split-model inference | ✅✅ end-to-end validated. Synthetic: 1.69× dual-lane on 1 Gbit ([`poc/09`](poc/09_dual_lane.py)). Real workflow: [comfyui-mesh](https://github.com/shootthesound/comfyui-mesh) FLUX.2 Klein 9B at 1024² in 4.38 s (NVENC) vs 7.20 s (raw) on RTX 5090 + RTX 4090 over gigabit — 1.64× end-to-end, widening to 2.07× at 1536² |
