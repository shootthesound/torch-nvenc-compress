...
| 20 | [`20_heat_equation_pframe.py`](20_heat_equation_pframe.py) | **Beyond ML**: 2D heat-equation simulator running 1000 timesteps. Encodes the trajectory two ways — every step as its own IDR vs one IDR + 999 P-frames — to measure how much extra compression NVENC's P-frame *temporal* chain extracts from a workload with real frame-to-frame coherence. **Measured ~24× P-frame win over I-only** (1 MB → 41 KB for the 1000-step trajectory) at PSNR 52 dB. Validates the broader claim that NVENC's inter-frame mode is a free delta-codec for any GPU workload producing temporally coherent state — CFD, FEM, weather, MD, progressive renderers, sci-viz streaming. | No |
| 21 | [`21_vortex_street_pframe.py`](21_vortex_street_pframe.py) | **Pushing the temporal claim onto a hard CFD test**: 2D incompressible Navier-Stokes (Stam stable fluids), Re=200, cylinder wake producing a Von Kármán vortex street over 1500 timesteps. Same comparison as poc/20 — but where heat is monotonic diffusion (no actual motion), this has translation AND nonlinear vortex shedding (motion estimation has to handle pattern *changes*, not just translations). **Measured ~9.6× P-frame win over I-only** (2.6 MB → 274 KB), reduced from heat's 24× because the dynamics are fundamentally harder. Visual reconstruction at PSNR ~22 dB (limited by outlier clipping at the cylinder edge; mean-abs-error across the field is 0.001). Validates the temporal-compression argument for the CFD / aerospace / weather audiences. Saves a vortex-shedding strip + reconstruction comparison + per-step-bytes plot to `docs/figures/`. | No |
